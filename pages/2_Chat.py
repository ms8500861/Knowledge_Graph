...
if user_input:
    ...
    try:
        ...
        print("passed step1: result", results)      
        cypher_step = results['intermediate_steps']
        print("passed step2: result", cypher_step)
        print("length of cypher step:", len(cypher_step))

        if len(cypher_step) > 0 and 'query' in cypher_step[0]:
            generated_query = cypher_step[0]['query']
            print("passed step3: generated query:", generated_query)
            st.session_state.cypher.append(generated_query)

            # Execute the query on Neo4j
            try:
                db_result = run_query(generated_query)
                print("passed step4: Database result:", db_result)

                # Store the full result
                st.session_state.database_results.append(db_result)

                # 👇 Add result to bot response (so it shows in chat bubble)
                import pandas as pd
                try:
                    df = pd.DataFrame(db_result)
                    df.reset_index(drop=True, inplace=True)  # remove index column
                    st.session_state.generated.append(df.to_markdown(index=False))
                except Exception:
                    st.session_state.generated.append(str(db_result))

            except Exception as e:
                st.session_state.database_results.append(f"Error running query: {e}")
                st.session_state.generated.append(f"Error running query: {e}")

        else:
            st.session_state.cypher.append('')
            st.session_state.database_results.append('')
            st.session_state.generated.append("I couldn't generate a valid query.")
...
if st.session_state['database_results']:
    db_result = st.session_state['database_results'][-1]
    if isinstance(db_result, list) or isinstance(db_result, dict):
        st.json(db_result)  # if results are dict/list
    else:
        try:
            import pandas as pd
            df = pd.DataFrame(db_result).reset_index(drop=True)
            df.index = [''] * len(df)  # 👈 Hide index labels completely
            st.table(
                df.style.set_table_styles([
                    {'selector': 'thead th', 'props': [('background-color', '#018BFF'), ('color', 'white'), ('font-weight', 'bold'), ('text-align', 'center')]},
